...
class Label { // TODO: add check to ensure labels have NO children
  ...
  size (renderContext, { text, size, font }, childBox) {
    renderContext.font = `${size}px ${font}`
    const textMetrics = renderContext.measureText(text)
    const newBox = {
      x: 0,
      y: 0,
      width: textMetrics.width, // when we have an ancestor with the deferRender prop,
      ...
      for (let box of this.childBoxes) {
        ...
        switch (mode) {
          ...
          case 'diagonal':
            ...
            break
          default:
            console.error('invalid layout mode in spacedLine:', mode)
        }
      }
      ...
      if (mode === 'horizontal') {
        switch (align) {
          ...
          case 'right':
            ...
            break
          default:
            console.error('invalid alignment mode in spacedLine:', align)
        }
